...
// Step 7-12 line builders — plain-text mirrors of each agent's Notion
// summary blocks (buildProductBugSummaryBlocks, buildFilmAISummaryBlocks,
// etc. in lib/*Agent.js), kept self-contained here since Docs API
// formatting works completely differently from Notion blocks (same pattern
// as buildSalesLines/buildRailwayLines above). Each one degrades to a single
// "not connected" line when the summary is null — mirrors the missing-data
// handling already used throughout this file.
function buildProductLines(summary) {
  if (!summary) return ['Product/Bug Agent not connected in this run.'];
  const lines = [
    `Status: ${summary.status}`,
    `Critical bugs open: ${summary.criticalBugs.length} — High: ${summary.highBugs.length} — New feedback: ${summary.newFeedbackCount}`,
  ];
  if (summary.recommendedFixToday) lines.push(`Recommended fix today: ${summary.recommendedFixToday}`);
  if (summary.repeatedAreas && summary.repeatedAreas.length > 0) {
    lines.push(`Repeated feedback areas: ${summary.repeatedAreas.join(', ')}`);
  }
  return lines;
}

function buildFilmAILines(summary) {
  if (!summary) return ['Film AI roadmap not connected in this run.'];
  const lines = [
    `Status: ${summary.status}`,
    `Tasks — Done: ${summary.doneCount}, Open: ${summary.openCount}, Blocked: ${summary.blockedCount}, Total: ${summary.totalTasks}`,
    `Next up: ${summary.nextUp}`,
  ];
  if (summary.criticalOpen && summary.criticalOpen.length > 0) {
    lines.push(`Critical open: ${summary.criticalOpen.map((t) => t.task).join('; ')}`);
  }
  return lines;
}

function buildCoachSalesLines(summary) {
  if (!summary) return ['Coach Sales pipeline not connected in this run.'];
  const t = summary.target;
  const lines = [
    `Status: ${summary.status}`,
    `Leads: ${summary.totalLeads} total (${summary.activeLeadCount} active, ${summary.wonCount} won, ${summary.lostCount} lost)`,
    `Progress toward first-sales target: ${summary.totalLeads}/${t.leads} leads, ${summary.outreachDraftCount}/${t.outreachDrafts} outreach drafts, ` +
      `${summary.conversationCount}/${t.conversations} conversations, ${summary.demoCount}/${t.demos} demo(s) booked`,
  ];
  if (summary.outreachNeedsApprovalCount > 0) {
    lines.push(`${summary.outreachNeedsApprovalCount} outreach draft(s) awaiting approval — nothing sent automatically.`);
  }
  return lines;
}

function buildSocialLines(summary) {
  if (!summary) return ['Social Media Agent not connected in this run.'];
  return [
    `Status: ${summary.status}`,
    `${summary.totalPosts} post(s) tracked, ${summary.totalViews} total views, avg engagement ${summary.avgEngagementRate !== null ? summary.avgEngagementRate + '%' : 'N/A'}`,
    ...
    `Social Inbox: ${summary.totalInboxItems} total (${summary.needsReplyBacklogCount} need a reply, ${summary.leadCount} leads, ${summary.inboxDraftsAwaitingApproval} draft reply(ies) awaiting approval)`,
  ];
}

function buildApprovalActionLines(summary) {
  if (!summary) return ['Approval Action Agent not connected in this run.'];
  return [
    `Status: ${summary.status}`,
    `Executed today: ${summary.executedTodayCount} — Failed: ${summary.failedCount} — Waiting: ${summary.waitingCount} — Blocked for safety: ${summary.blockedForSafetyCount}`,
  ];
}

function buildWeeklyStrategyLines(summary) {
  if (!summary) {
    return ['No Weekly Strategy Review in the last 8 days — run POST /run-weekly-strategy-review.'];
  }
  const lines = [`Week: ${summary.week} — Status: ${summary.status}`];
  if (summary.topPriorities) lines.push(`Top priorities: ${summary.topPriorities}`);
  if (summary.decisionsNeeded) lines.push(`Decisions needed: ${summary.decisionsNeeded}`);
  if (summary.risks) lines.push(`Risks: ${summary.risks}`);
  return lines;
}
...
function buildDocSections({
  ...
  return [
    ...
    { type: 'heading', text: 'Product / Bugs' },
    { type: 'bullets', items: buildProductLines(productBugSummary) },
    { type: 'heading', text: 'Film AI Roadmap' },
    { type: 'bullets', items: buildFilmAILines(filmAISummary) },
    { type: 'heading', text: 'Coach Sales' },
    { type: 'bullets', items: buildCoachSalesLines(coachSalesSummary) },
    { type: 'heading', text: 'Social Media' },
    { type: 'bullets', items: buildSocialLines(socialMediaSummary) },
    { type: 'heading', text: 'Approval Action Agent' },
    { type: 'bullets', items: buildApprovalActionLines(approvalActionSummary) },
    { type: 'heading', text: 'Weekly Strategy' },
    { type: 'bullets', items: buildWeeklyStrategyLines(weeklyStrategySummary) },
